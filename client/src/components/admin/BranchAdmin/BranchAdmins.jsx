...
const BranchAdmins = () => {
  ...
  const [getBranchesByCompany, { data: branchesData }] =
    useGetBranchesByCompanyMutation();

  const { data, isLoading, refetch } = useGetAllBranchAdminsQuery({
    page: currentPage,
    limit,
  ...
  const [deleteBranchAdmin, { isSuccess, isError }] =
    useDeleteBranchAdminMutation();
  ...
  const handleCompanyChange = async (val) => {
    setCompany(val);
    setBranch("all");
    if (val !== "all") await getBranchesByCompany(val);
  };
      ...
      <div className="md:p-6 p-2">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          ...
          <div className="flex gap-4 items-center flex-wrap">
            ...
            />
            <Button onClick={() => navigate("/admin/create-branch-admin")}
        ...
        <div className="bg-white/80 dark:bg-gray-900/80 rounded-2xl shadow-lg overflow-x-auto border border-gray-100 dark:border-gray-800 backdrop-blur-md">
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-200 text-center">
              ...
              ) : data?.branchAdmins?.length ? (
                data.branchAdmins.map((admin, i) => (
                  <tr
                    key={admin._id}
                    className={
                      i % 2 === 0
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              permanently delete the branch admin and remove
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() => deleteBranchAdmin(admin._id)}
                            >
                              Delete
                            </AlertDialogAction>
                          </AlertDialogFooter>
        ...
        {/* Drawer moved outside the table, styled like Drivers.jsx */}
        <Drawer
          title={
        ...
        >
          {!selectedAdmin ? (
            ...
            <div className="p-6 space-y-6 overflow-x-hidden">
              <InfoCard icon={User} title="Admin Information">
                {selectedAdmin?.photoUrl && (
                  <img
                    src={selectedAdmin?.photoUrl}
                    alt="Profile"
                    className="w-24 h-24 rounded-full object-cover mx-auto border mb-4"
                  />
                )}
                <InfoRow label="Name" value={selectedAdmin.name} icon={User} />
                <InfoRow
                  label="Email"
                ...
                <InfoRow
                  label="Status"
                  value={selectedAdmin.status ? "Active" : "Inactive"}
                  icon={User}
                />
                <InfoRow
                  label="Company"
