...
import { Loader2, Globe2, EyeIcon } from "lucide-react";
...
const Pincodes = () => {
  ...
  const { data, isLoading, refetch } = useGetAllPincodesQuery({
    page: currentPage,
    limit,
    search: debouncedSearchQuery,
  });

  const [deletePincode, { isSuccess, isError }] = useDeletePincodeMutation();
  const [getPincodeById] = useGetPincodeByIdMutation();
  const [open, setOpen] = useState(false);
  const [selectedPincode, setSelectedPincode] = useState(null);

  const handleViewPincode = async (id) => {
    setOpen(true);
    try {
      const { data } = await getPincodeById(id);
      if (data?.success) setSelectedPincode(data.data);
    } catch (error) {
      ...
      <div className="md:p-6 p-2">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          ...
          <div className="flex gap-4 items-center flex-wrap">
            ...
            />
            <Button onClick={() => navigate("/admin/create-pincode")}
              ...
              {isLoading ? (
                ...
                data.pincodes.map((pincode, i) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-[#202020] dark:text-[#FFD249]">{pincode.code}</td>
                    <td className="p-3 text-[#202020] dark:text-[#FFD249]">{pincode.locality?.name}</td>
                    <td className="p-3 flex gap-2 items-center justify-center">
                      <Button
                        ...
                        onClick={() => handleViewPincode(pincode._id)}
                      >
                          ...
                          <div className="flex items-center gap-4 py-2">
                            ...
                            <div className="min-w-0">
                              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 truncate max-w-[160px]">
                                {selectedPincode?.code || "Pincode Details"}
                              </h2>
                            </div>
                          </div>
                        }
                      ...
                      >
                        {!selectedPincode ? (
                          ...
                          <div className="p-6 space-y-6 overflow-x-hidden">
                            <InfoCard icon={Globe2} title="Pincode Information">
                              <InfoRow label="Code" value={selectedPincode.code} icon={Globe2} />
                              <InfoRow label="Locality" value={selectedPincode.locality?.name} icon={Globe2} />
                              <InfoRow label="Status" value={selectedPincode.status ? 'Active' : 'Inactive'} icon={Globe2} />
                            </InfoCard>
                          </div>
                        )}
                      </Drawer>
                      <Button
                        ...
                        onClick={() =>
                          navigate("/admin/update-pincode", {
                            state: { pincodeId: pincode._id },
                          })
                        }
                      >
          ...
          <div className="border-t border-gray-200 px-4 py-3 flex flex-col lg:flex-row lg:items-center lg:justify-between bg-white/70 dark:bg-gray-900/70 rounded-b-2xl">
            <div className="mb-4 lg:mb-0">
              <p className="text-sm text-[#202020] dark:text-[#FFD249]">
                Showing {data?.pincodes?.length ? (data?.page - 1) * data?.limit + 1 : 0} to {Math.min(data?.page * data?.limit, data?.total || 0)} of <span className="font-medium">{data?.total || 0}</span> entries
              </p>
            </div>
            <div>
              {data?.totalPages > 1 && (
