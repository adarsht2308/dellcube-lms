...
import { Loader2, EyeIcon, Globe2, MapPin } from "lucide-react";
...
const Countries = () => {
  ...
  const [deleteCountry, { isSuccess, isError }] = useDeleteCountryMutation();
  const [getCountryById] = useGetCountryByIdMutation();

  const [open, setOpen] = useState(false);
  const [selectedCountry, setSelectedCountry] = useState(null);

  const handleViewCountry = async (id) => {
    setOpen(true);
    try {
      const { data } = await getCountryById(id);
      if (data?.success) setSelectedCountry(data.country);
    } catch (error) {
      ...
      <div className="md:p-6 p-2">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          ...
          <div className="flex gap-4 items-center flex-wrap">
            ...
            />
            <Button onClick={() => navigate("/admin/create-country")}
              ...
              {isLoading ? (
                ...
                data.countries.map((country, i) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-[#202020] dark:text-[#FFD249]">{country.name}</td>
                    <td className="p-3 text-[#202020] dark:text-[#FFD249]">{country.code}</td>
                    <td className="p-3 flex gap-2 items-center justify-center">
                      ...
                      <Drawer
                        title={selectedCountry?.name || "Country Info"}
                        placement="right"
                        width={360}
                        onClose={() => {
                          setOpen(false);
                      ...
                      >
                        {!selectedCountry ? (
                          ...
                          <InfoCard icon={Globe2} title={selectedCountry.name}>
                            <InfoRow label="Country Name" value={selectedCountry.name} icon={Globe2} />
                            <InfoRow label="Country Code" value={selectedCountry.code} icon={MapPin} />
                            <InfoRow label="Status" value={selectedCountry.status ? 'Active' : 'Inactive'} icon={Globe2} />
                            <InfoRow label="Created At" value={selectedCountry.createdAt ? new Date(selectedCountry.createdAt).toLocaleString() : undefined} />
                            <InfoRow label="Updated At" value={selectedCountry.updatedAt ? new Date(selectedCountry.updatedAt).toLocaleString() : undefined} />
                            {/* Add more InfoRows as needed */}
                          </InfoCard>
                        )}
                      </Drawer>
                      <Button
          ...
          <div className="border-t border-gray-200 px-4 py-3 flex flex-col lg:flex-row lg:items-center lg:justify-between bg-white/70 dark:bg-gray-900/70 rounded-b-2xl">
            <div className="mb-4 lg:mb-0">
              <p className="text-sm text-[#202020] dark:text-[#FFD249]">
                Showing {data?.countries?.length ? (data?.page - 1) * data?.limit + 1 : 0} to {Math.min(data?.page * data?.limit, data?.total || 0)} of <span className="font-medium">{data?.total || 0}</span> entries
              </p>
            </div>
            <div>
              {data?.totalPage > 1 && (
