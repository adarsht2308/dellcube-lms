...
const States = () => {
  ...
  const { data, isLoading, refetch } = useGetAllStatesQuery({
    page: currentPage,
    limit,
    search: debouncedSearchQuery,
  });

  const [deleteState, { isSuccess, isError }] = useDeleteStateMutation();
      ...
      <div className="md:p-6 p-2">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          ...
          <div className="flex gap-4 items-center flex-wrap">
            ...
            />
            <Button onClick={() => navigate("/admin/create-state")}
          ...
          <table className="min-w-full text-sm">
            ...
            <thead className="bg-[#FFD249]/20 dark:bg-[#FFD249]/10 text-center sticky top-0 z-10">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold uppercase text-[#202020] dark:text-[#FFD249] tracking-wider">Country</th>
              ...
              {isLoading ? (
                ...
                data.data.map((state, i) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-[#202020] dark:text-[#FFD249]">{state.name}</td>
                    <td className="p-3 text-[#202020] dark:text-[#FFD249]">{state.country?.name}</td>
                    <td className="p-3 flex gap-2 items-center justify-center">
                      ...
                      <Drawer
                        title={selectedState?.name || "State Info"}
                        placement="right"
                        width={360}
                      ...
                      >
                        {!selectedState ? (
                          ...
                          <InfoCard icon={Globe2} title={selectedState.name}>
                            <InfoRow label="State Name" value={selectedState.name} icon={MapPin} />
                            <InfoRow label="Country" value={selectedState.country?.name} icon={Globe2} />
                            <InfoRow label="Status" value={selectedState.status ? 'Active' : 'Inactive'} icon={Globe2} />
                          </InfoCard>
                        )}
                      </Drawer>
                      <Button
