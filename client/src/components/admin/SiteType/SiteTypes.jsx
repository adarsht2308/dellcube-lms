...
import { Loader2, EyeIcon, Tag, FileText, Calendar, CheckCircle, XCircle } from "lucide-react";
...
const SiteTypes = () => {
  ...
  const { data, isLoading, refetch } = useGetAllSiteTypesQuery({
    page: currentPage,
    limit,
  ...
  const [deleteSiteType, { isSuccess, isError }] = useDeleteSiteTypeMutation();
  const [open, setOpen] = useState(false);
  const [selectedSiteType, setSelectedSiteType] = useState(null);
  const [getSiteTypeById, { isLoading: getSiteTypeLoading }] =
    useGetSiteTypeByIdMutation();

  const handleView = async (siteTypeId) => {
    setOpen(true);

    try {
      const { data } = await getSiteTypeById(siteTypeId);
      if (data?.success) {
        setSelectedSiteType(data.siteType);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      toast.error("Failed to delete site type");
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#828083] dark:text-gray-400">Total Site Types</p>
              ...
              <div className="p-3 bg-[#FFD249]/20 dark:bg-[#FFD249]/10 rounded-xl">
                <Tag className="w-6 h-6 text-[#202020] dark:text-[#FFD249]" />
              </div>
            </div>
          </div>
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-lg">
        ...
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-lg">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex items-center gap-4">
              <h2 className="text-2xl font-bold text-[#202020] dark:text-[#FFD249]">All Site Types</h2>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                />
                <Tag className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-[#828083] dark:text-gray-400" />
              </div>
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="w-[120px] bg-white/80 dark:bg-gray-700/80 border-gray-200/50 dark:border-gray-600/50">
              ...
              <Button
                onClick={() => navigate("/admin/create-site-type")}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan="6" className="text-center py-10 text-[#828083]">
                    <Tag className="w-8 h-8 mx-auto text-[#828083]" />
                    <p className="text-[#828083] font-medium">No Site Types Available</p>
                    <p className="text-sm text-[#828083]">Add a new site type to begin</p>
                  </td>
                </tr>
              ) : (
                data?.siteTypes?.map((siteType, index) => (
                  <tr
                    key={siteType._id}
                    className={
                      index % 2 === 0
                  ...
                  >
                    ...
                    <td className="p-3 text-[#202020] dark:text-[#FFD249] font-semibold">{siteType.name}</td>
                    <td className="p-3 text-[#202020] dark:text-[#FFD249] max-w-xs truncate">{siteType.desc}</td>
                    <td className="p-3 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold text-center ${siteType.status ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"}`}>{siteType.status ? "Active" : "Inactive"}</span>
                    </td>
                    <td className="p-3 text-[#202020] dark:text-[#FFD249] text-center">{new Date(siteType.createdAt).toLocaleDateString()}</td>
                    <td className="p-3 flex gap-2 items-center justify-center">
                      <Button
                        ...
                        onClick={() => handleView(siteType._id)}
                        title="View Details"
                      >
                      ...
                      <Button
                        className="p-2 rounded-full bg-[#FFD249]/30 text-[#202020] hover:bg-[#FFD249]/60 dark:text-[#FFD249]"
                        onClick={() =>
                          navigate("/admin/update-site-type", {
                            state: { siteTypeId: siteType._id },
                          })
                        }
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>Delete Site Type</AlertDialogTitle>
                            <AlertDialogDescription>
                              Are you sure you want to delete "{siteType.name}"? This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() => handleDelete(siteType._id)}
        ...
        >
          {getSiteTypeLoading ? (
            ...
            <div className="space-y-6">
              <InfoCard icon={Tag} title="Basic Information">
                <InfoRow label="Name" value={selectedSiteType.name} icon={Tag} />
                <InfoRow label="Description" value={selectedSiteType.desc} icon={FileText} />
                <InfoRow
                  label="Status"
                  value={selectedSiteType.status ? "Active" : "Inactive"}
                  icon={selectedSiteType.status ? CheckCircle : XCircle}
                />
              </InfoCard>
              ...
              <InfoCard icon={Calendar} title="Timestamps">
                <InfoRow
                  label="Created At"
                  value={new Date(selectedSiteType.createdAt).toLocaleString()}
                  icon={Calendar}
                />
                <InfoRow
                  label="Updated At"
                  value={new Date(selectedSiteType.updatedAt).toLocaleString()}
                  icon={Calendar}
                />
              </InfoCard>
            </div>
            ...
            <div className="text-center py-8 text-gray-500">
              No site type details available
            </div>
          )}
        </Drawer>
      </div>
