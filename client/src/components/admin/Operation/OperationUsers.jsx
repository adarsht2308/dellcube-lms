...
const OperationUsers = () => {
  ...
  const { data, isLoading, refetch } = useGetAllOperationUsersQuery({
    page,
    limit,
  ...
  const [deleteOperationUser] = useDeleteOperationUserMutation();
  ...
  const handleDelete = async (id) => {
    const res = await deleteOperationUser(id);
    if (res?.data?.success) {
      ...
      <div className="md:p-6 p-2">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          ...
          <div className="flex gap-4 items-center flex-wrap">
            <Input
              placeholder="Search operation user..."
              value={search}
            ...
            <Button onClick={() => navigate("/admin/create-operation-user")}
        ...
        <div className="bg-white/80 dark:bg-gray-900/80 rounded-2xl shadow-lg overflow-x-auto border border-gray-100 dark:border-gray-800 backdrop-blur-md">
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-200 text-center">
              ...
              ) : data?.operationUsers?.length ? (
                data.operationUsers.map((user, i) => (
                  <tr
                    key={user._id}
                    className={
                      i % 2 === 0
                  ...
                  >
                    ...
                    <td className="p-3 flex gap-2 items-center justify-center">
                      ...
                      <Button
                        className="p-2 rounded-full bg-[#FFD249]/30 text-[#202020] hover:bg-[#FFD249]/60"
                        onClick={() =>
                          navigate("/admin/update-operation-user", {
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>Delete Operation User?</AlertDialogTitle>
                            <AlertDialogDescription>
                              This action cannot be undone. This will permanently delete the operation user and remove their data from our servers.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
          ...
          <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-800 text-sm text-[#202020] dark:text-[#FFD249] text-center lg:text-left">
            Showing {data?.operationUsers?.length ? (data?.page - 1) * data?.limit + 1 : 0} to {Math.min(data?.page * data?.limit, data?.total || 0)} of <span className="font-medium">{data?.total || 0}</span> entries
          </div>
        </div>
            ...
            title={
              <div className="flex items-center gap-4 py-2">
                ...
                <div className="ml-auto">
                  <span className={`inline-flex items-center gap-2 px-3 py-2 rounded-full font-medium ${selectedUser?.status ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400' : 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400'}`}>{selectedUser?.status ? 'Active' : 'Inactive'}</span>
                </div>
              </div>
            }
            placement="right"
          ...
          >
            {!selectedUser ? (
              ...
              <div className="p-6 space-y-6 overflow-x-hidden">
                <InfoCard icon={UserRound} title="Operation User Information">
